...
$link = mysqli_connect ( $_SESSION ['mysql'] ['server'], $_SESSION ['mysql'] ['user'], $_SESSION ['mysql'] ['password'] );
...
if (! $database) {
	echo "<div class='message ui error' style='max-width:800px'><i class='close icon'></i><div class='header'>Info</div>Keine Datenbank definiert</div><br>";
	exit ();
}
...
if ($database == 'ssi_smart_all') {
	...
	$array_database = array ("ssi_faktura","ssi_faktura93","ssi_faktura94" );
} else
...
if ($set_table && $field) {
	...
	if (preg_match ( "/^select/i", $query )) {
		...
		// Ausführen anderer Ausführungen wie Delete,...
		echo "<div class='message ui' style='max-height:400px; overflow: auto;'>";
		echo "<pre>";
		echo $output;
		echo "</pre>";
		echo "</div>";
...
/**
 * *****************************************
 * Abarbeitung auch von mehreren Datenbanken
 * *****************************************
 */
...
foreach ( $array_database as $database ) {
	...
	if ($checkbox_table_utf8) {
		...
		if (! $utf8_table) {
			...
			$query_message [] = "($count1 Ausführungen) Table 'alle' von $database auf UT8 konvertiert";
		} else {
			// Einzelnes Table konvertieren
			$query_array [] = "ALTER TABLE `$utf8_table` CONVERT TO CHARACTER SET utf8 COLLATE utf8_general_ci";
			$query_message [] = "Table '$utf8_table' von $database auf UT8 konvertiert (1 Ausführung)";
		}
	}

	// Aufruf function replace "Sonderzeichen"
	if ($utf8_column && $utf8_table) {
		...
		if ($checkbox_replace_specialcaracter) {
			if ($utf8_column == 'all') {
				...
				$query_message [] = "Specialcaracter für $utf8_table (alle) umgwandelt ($count2 Ausführungen)";
			} else {
				$query_array = array_merge ( fix_utf8_column ( $utf8_table, $utf8_column ), $query_array );
				$query_message [] = "Specialcaracter für $utf8_table ($utf8_column) umgwandelt (1 Ausführung)";
			}
		}
		...
		if ($checkbox_replace_entities) {
			if ($utf8_column == 'all') {
				...
				$query_message [] = "Sonderzeichen für $utf8_table (all) umgwandelt ($count3 Ausführungen)";
			} else {
				$query_array = array_merge ( fix_utf8_entities_column ( $utf8_table, $utf8_column ), $query_array );
				$query_message [] = "Sonderzeichen für $utf8_table ($utf8_column) umgwandelt (1 Ausführung)";
			}
		}
	}
	...
	if ($checkbox_optimize_tables) {
		...
		$query_message [] = "Optimierung für Db '$database' wurde durchgeführt ($count4 Ausführungen)";
	}
	...
	if (is_array ( $query_array )) {
		foreach ( $query_array as $key => $query ) {
			...
			if ($error_query) {
				$output .= "<br><i><font color=grey>$query</font></i><br><font color=red>$database -> $error_query</font><br>";
			} else {
				$count ++;
				//$output .= "<i>$query</i><br>";

				// in Db speichern
				// mysqli_query ( $link, "INSERT INTO `ssi_company`.`log_service_sql` SET query='$query', `database`='$database' " ) or die ( mysqli_error ($GLOBALS['mysqli']) );
			}
		}
		foreach ( $query_message as $message ) {
			$output .= " • <i>" . $message . "</i><br>";
		}

		$output .= "<hr><font color=green>Ausführung von <b>$count</b> Querys für \"$database\" waren erfolgreich!</font>";
	}
	if (! $message) {
		$output = "Keine Sql-Befehle abzuarbeiten";
	}
}

echo "<div class='message ui' style='max-width:1000px;'><i class='close icon'></i><div class='header'>Info</div><br>" . nl2br ( $output ) . "</div>";
exit ();
