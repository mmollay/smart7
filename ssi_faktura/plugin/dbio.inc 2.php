...
class DBIO {
	...
	function __construct($host, $user, $pass, $db) {
		$this->dbconn = mysql_connect ( $host, $user, $pass ) or die ( "cannot connect to mySQL host $host" );
	...
	function import($filename, $tablename, $column_list, $separator = ';', $iconv_source = 'utf8', $clear_table = false, $set_passwords = false, $pwdfield = null, $notifyuser = false, $emailfield = null, $email_subj = null, $email_tplt = null, $test_mode = true) {
		...
		if ($clear_table) {
			...
			$GLOBALS['mysqli']->query ( $sql, $this->dbconn ) or die ( "cannot clear table $tablename" );
		}
		...
		while ( ! feof ( $fp ) ) {
			...
			if ($line != "") {
				...
				$GLOBALS['mysqli']->query ( $sql, $this->dbconn ) or die ( "cannot execute query: $sql" );
	...
	function export($filename, $tablename, $column_array, $separator = ';', $filter = null) {
		...
		$result = $GLOBALS['mysqli']->query ( $sql, $this->dbconn );
	...
	function __destruct() {
		$GLOBALS['mysqli']->close ( $this->dbconn );
		$this->dbconn = null;
	}
}
