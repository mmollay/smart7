...
function removePlaceholderImages() {
	var placeholderImages = document.querySelectorAll('.pswp__img--placeholder');
...
// Funktion zum Aktualisieren des src-Attributs des Bildes
function updateImageSrc() {
	...
	images.forEach(function(image) {
		...
		image.src = baseUrl + '?timestamp=' + newTimestamp;
		console.log(image.src);
	});
}

// Funktion, die überprüft, ob das Popup geöffnet ist
function checkForPopup() {
	var htmlElement = document.documentElement;
	if (htmlElement.classList.contains('popup-open')) {
		removePlaceholderImages();
		
		// Setze eine Verzögerung von 0,5 Sekunden, bevor updateImageSrc() aufgerufen wird
		setTimeout(function() {
			console.log('checkForPopup');
			updateImageSrc();
		}, 400); // Verzögerung in Millisekunden
	}
}

// MutationObserver zur Überwachung von Änderungen am <html>-Element
...
// Konfiguration des Observers: Überwachung von Attributänderungen am <html>-Element
