		...
		$(this)
			.api({
				...
				onRequest: function () {
					...
					$('#' + key).modal({ onHidden: function () { $('#' + key + ">.content").empty(); }, closable: false, allowMultiple: true, observeChanges: true, autofocus: autofocus }); // closable:false
					$('#' + key).modal('show');
					//$('#'+key).modal({content:'<div class='ui loading segment'><br><br><br><br><br><br><br><br><br><br><br><br></div>'});
