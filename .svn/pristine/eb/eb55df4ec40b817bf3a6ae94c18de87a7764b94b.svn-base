<?php
include (__DIR__ . '/../exclusion_periods.php');
...
$arr['mysql'] = array(
    'table' => "ssi_trader.deposits AS d
                ...
                LEFT JOIN ssi_trader.orders AS o ON d.account = o.account AND o.trash = 0 AND  $exclusionClause",
    ...
    'field' => "d.deposit_id, 
                ...
                CASE
                    WHEN SUM(o.profit) > 0 THEN SUM(o.profit) * d.positive_multiplier
                    ELSE SUM(o.profit) * d.negative_multiplier
