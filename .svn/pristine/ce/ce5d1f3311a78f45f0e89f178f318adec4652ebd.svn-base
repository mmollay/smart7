...
$tokens = getTokensFromDatabase($mysqli);
...
if ($result = mysqli_query($mysqli, $serverQuery)) {
    while ($row = mysqli_fetch_assoc($result)) {
        processServerRow($row, $mysqli, $response, $tokens);
    }
...
function getTokensFromDatabase($mysqli)
{
    $tokens = [];
    $query = "SELECT server_id, token FROM servers WHERE active = 1";
    $result = mysqli_query($mysqli, $query);
    if ($result) {
        while ($row = mysqli_fetch_assoc($result)) {
            $tokens[$row['server_id']] = $row['token'];
        }
...
function processServerRow($row, $mysqli, &$response, $tokens)
{
    $apiUrl = $row['url'] . "/history";
    $token = $tokens[$row['server_id']] ?? null;

    if (!$token) {
        $response['errors'][] = ['message' => "Fehler beim Abrufen des Tokens für Server ID: " . $row['server_id'], 'type' => 'error'];
        return;
    }
...
// Die restlichen Funktionen (processOrders, insertOrders, assignStrategiesFromAssignments) 
// bleiben unverändert wie in Ihrem ursprünglichen Code
