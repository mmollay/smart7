...
-add: list -> class_total //farbe für Table Zeile setzen
-add: list -> group //Dropdown vor groups view<<>>
-add: list -> mysql -> total -> "Select SUM(price)....{where}" //platzhalter für where Spezialabfrage für SUM 
-fix: form -> dropdown forms mit unterschiedlichen class können gleiche ID verwenden
