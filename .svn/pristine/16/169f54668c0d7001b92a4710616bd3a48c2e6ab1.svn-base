...
switch ($_POST ['list_id']) {
	case 'session' :
		$GLOBALS ['mysqli']->query ( "DELETE FROM usrdb_intubspe.session WHERE session_id = '{$_POST['delete_id']}' LIMIT 1" ) or die ( mysqli_error ( $GLOBALS ['mysqli'] ) );
		break;
	case 'user' :
		$GLOBALS ['mysqli']->query ( "DELETE FROM usrdb_intubspe.user WHERE user_id = '{$_POST['delete_id']}' LIMIT 1" ) or die ( mysqli_error ( $GLOBALS ['mysqli'] ) );
		break;
}