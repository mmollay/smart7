...
if ($_POST ['clone_id']) {
	$clone_fields = "user_id,title,text,modus,from_name,from_email,error_email,report_email,smtp_server,smtp_user,smtp_password,smtp_secure,smtp_port,test_email,sender_id,tag_id,from_id,all_user,without_footline";
...
$arr ['ajax'] = array ('onLoad' => $onload,'success' => "
	...
	else if (data == 'ok') { $('.modal.ui').modal('hide');  table_reload('newsletter_list'); }",'dataType' => "html" );
...
$arr ['field'] ['array_contact_id'] = array ('value' => $array_contact_id,'tab' => 'first','type' => 'multiselect','label' => "Zusätzliche User <div class='ui label mini green'>versenden</div>",
		// 'array' => call_array_contact ($_POST['session_id']),
		'url' => 'inc/get_array.php?search={query}','class' => 'search','info' => 'hier können noch weitere einzelne Kontakte hinzugefügt werden.' );

$arr ['field'] ['array_contact_id_minus'] = array ('value' => $array_contact_id_minus,'tab' => 'first','type' => 'multiselect','label' => "Diese User <div class='ui label mini red'>ausschließen</div>",
		// 'array' => call_array_contact ($_POST['session_id']) ,
		'url' => 'inc/get_array.php?search={query}','class' => 'search','info' => 'hier können noch weitere einzelne Kontakte hinzugefügt werden.' );
...
$add_js .= "<script type=\"text/javascript\" charset=\"utf-8\" src=\"js/form_newsletter.js\"></script>";
// $add_js .= "<script>appendScript('js/form_newsletter.js');</script>";
