...
function set_new_tag_from_session($session_id, $event) {
	...
	$array = mysqli_fetch_array ( $query );
	$tag_id = $array ['tag_id'];
	$GLOBALS ['mysqli']->query ( "DELETE FROM contag2tag WHERE tag_id = '$tag_id' " );
	// Wenn Tag nicht exisitiert dann neuen Tag anlegen
	if (! $tag_id) {
		// Tag anlegen
		$GLOBALS ['mysqli']->query ( "INSERT INTO tag SET title = '$title', description = '',  user_id = '{$_SESSION['user_id']}', session_id = '$session_id' , event = '$event' " ) or die ( mysqli_error ( $GLOBALS ['mysqli'] ) );
	...
	while ( $array = mysqli_fetch_array ( $query ) ) {
		...
		$GLOBALS ['mysqli']->query ( "REPLACE INTO contact2tag SET tag_id = '$tag_id', contact_id = '$contact_id', activate = 1 " ) or die ( msysql_error () );
	}
	return $tag_id;
}
